...
var SIWSSBSessionTableColumns = struct {
	ID        string
	Token     string
	MemberID  string
	CreatedAt string
}{
	ID:        "SIWSSB_sessions.id",
	Token:     "SIWSSB_sessions.token",
	MemberID:  "SIWSSB_sessions.member_id",
	CreatedAt: "SIWSSB_sessions.created_at",
}
...
func (r *sIWSSBSessionR) GetMember() *Member {
	if r == nil {
		return nil
	}
	return r.Member
...
var (
	...
	sIWSSBSessionColumnsWithoutDefault = []string{"token", "member_id"}
	sIWSSBSessionColumnsWithDefault    = []string{"id", "created_at"}
	sIWSSBSessionPrimaryKeyColumns     = []string{"id"}
	sIWSSBSessionGeneratedColumns      = []string{"id"}
)

type (
	// SIWSSBSessionSlice is an alias for a slice of pointers to SIWSSBSession.
	// This should almost always be used instead of []SIWSSBSession.
...
func (sIWSSBSessionL) LoadMember(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSIWSSBSession interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeSIWSSBSession.(*SIWSSBSession)
		if !ok {
			object = new(SIWSSBSession)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeSIWSSBSession)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeSIWSSBSession))
			}
		}
	} else {
		s, ok := maybeSIWSSBSession.(*[]*SIWSSBSession)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeSIWSSBSession)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeSIWSSBSession))
			}
		}
	}
...
func SIWSSBSessions(mods ...qm.QueryMod) sIWSSBSessionQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"SIWSSB_sessions\".*"})
	}

	return sIWSSBSessionQuery{q}
}
...
func FindSIWSSBSession(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*SIWSSBSession, error) {
	...
	if err = sIWSSBSessionObj.doAfterSelectHooks(ctx, exec); err != nil {
		return sIWSSBSessionObj, err
	}
...
func (o *SIWSSBSession) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		wl = strmangle.SetComplement(wl, sIWSSBSessionGeneratedColumns)
...
func (o *SIWSSBSession) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no SIWSSB_sessions provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	sIWSSBSessionUpsertCacheMut.RLock()
	cache, cached := sIWSSBSessionUpsertCache[key]
	sIWSSBSessionUpsertCacheMut.RUnlock()
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert SIWSSB_sessions, could not build update column list")
		}

		conflict := conflictColumns
		if len(conflict) == 0 {
			conflict = make([]string, len(sIWSSBSessionPrimaryKeyColumns))
			copy(conflict, sIWSSBSessionPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"SIWSSB_sessions\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(sIWSSBSessionType, sIWSSBSessionMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
			cache.retMapping, err = queries.BindMapping(sIWSSBSessionType, sIWSSBSessionMapping, ret)
			if err != nil {
				return err
			}
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert SIWSSB_sessions")
	}

	if !cached {
		sIWSSBSessionUpsertCacheMut.Lock()
		sIWSSBSessionUpsertCache[key] = cache
		sIWSSBSessionUpsertCacheMut.Unlock()
	}
...
// Exists checks if the SIWSSBSession row exists.
func (o *SIWSSBSession) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return SIWSSBSessionExists(ctx, exec, o.ID)
}
