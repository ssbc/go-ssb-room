...
type (
	...
	// This should generally be used opposed to []Pin.
	PinSlice []*Pin
...
func (o *Pin) Notices(mods ...qm.QueryMod) noticeQuery {
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"notices\".*"})
	}
...
func (o *Pin) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT \"%s\" FROM \"pins\" WHERE %s", strings.Join(returnColumns, "\",\""), strmangle.WhereClause("\"", "\"", 0, pinPrimaryKeyColumns))
		}
	...
	o.ID = int64(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == pinMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for pins")
	}
